                              ...
                              access_type_of_rpc_code &ac_type_of_rpc)
{
    for (const auto &rpc_code : rpc_codes) {
        ...
        dassert_f(code != TASK_CODE_INVALID, "invalid code = {}", rpc_code);
...
{
    dassert_f(policies.empty(), "Ranger policy list should not be empty.");
...
dsn::error_code ranger_resource_policy_manager::update_policies_from_ranger_service()
{
    ...
    LOG_AND_RETURN_NOT_OK(error_f,
                          pull_policies_from_ranger_service(&ranger_policies),
                          "Pull Ranger policies failed.");
    ddebug("Pull Ranger policies success.");

    auto err_code = load_policies_from_json(ranger_policies);
    if (err_code == dsn::ERR_RANGER_POLICIES_NO_NEED_UPDATE) {
        ddebug("Skip to update local policies.");
        ...
        LOG_AND_RETURN_NOT_OK(
            error_f, sync_policies_to_app_envs(), "Sync policies to app envs failed.");
        return dsn::ERR_OK;
    }
    LOG_AND_RETURN_NOT_OK(error_f, err_code, "Parse Ranger policies failed.");
...
dsn::error_code ranger_resource_policy_manager::load_policies_from_json(const std::string &data)
{
    ...
    */
    ...
    if (_local_policy_version == remote_policy_version) {
        ddebug_f("Ranger policy version: {}, no need to update.", _local_policy_version);
        return dsn::ERR_RANGER_POLICIES_NO_NEED_UPDATE;
    }

    if (_local_policy_version > remote_policy_version) {
        dwarn_f("Local Ranger policy version ({}) is larger than remote version ({}), please "
    ...
    _meta_svc->get_remote_storage()->create_node(
        ...
        [this, sync_task](dsn::error_code err) {
            if (err == dsn::ERR_OK || err == dsn::ERR_NODE_ALREADY_EXIST) {
                ddebug("Create Ranger policy meta root succeed.");
                sync_task->enqueue();
                return;
            }
            dassert_f(err == dsn::ERR_TIMEOUT,
                      "error to create Ranger policy meta root, error = {}",
                      err);
            derror("Create Ranger policy meta root timeout, retry later.");
...
{
    ddebug("Start to sync Ranger policies to remote storage.");

    dump_policies_to_remote_storage();
    ddebug("Dump Ranger policies to remote storage succeed.");

    update_cached_policies();
    ddebug("Update using resources policies succeed.");

    if (dsn::ERR_OK != sync_policies_to_app_envs()) {
        derror("Sync policies to app envs failed.");
    }
}
...
void ranger_resource_policy_manager::dump_policies_to_remote_storage()
{
    ...
    _meta_svc->get_remote_storage()->set_data(
        _ranger_policy_meta_root, value, LPC_USE_RANGER_ACCESS_CONTROL, [this](dsn::error_code e) {
            if (e == dsn::ERR_OK) {
                ddebug_f("Dump Ranger policies to remote storage succeed.");
                return;
            }
            // The return error code is not 'ERR_TIMEOUT', use assert here.
            dassert_f(e == dsn::ERR_TIMEOUT,
                      "error to dump Ranger policies to remote storage, error = {}",
                      e);
            derror("Dump Ranger policies to remote storage timeout, retry later.");
            dsn::tasking::enqueue(LPC_USE_RANGER_ACCESS_CONTROL,
                                  &_tracker,
...
dsn::error_code ranger_resource_policy_manager::sync_policies_to_app_envs()
{
    ...
    if (table_policies == _all_resource_policies.end()) {
        dinfo_f("DATABASE_TABLE level policy is empty, skip to sync app envs.");
        return dsn::ERR_OK;
    }
    ...
    LOG_AND_RETURN_NOT_OK(error_f, list_resp.err, "list_apps failed.");
    for (const auto &app : list_resp.infos) {
        ...
        for (const auto &policy : table_policies->second) {
            ...
            if (policy.database_names.count(database_name) == 0) {
                dassert(policy.database_names.count("*") != 0,
            ...
            if (policy.table_names.count(table_name) == 0) {
                dassert(policy.table_names.count("*") != 0,
        ...
        if (matched_database_table_policies.empty()) {
            ...
            LOG_AND_RETURN_NOT_OK(error_f, rpc.response().err, "del_app_envs failed.");
        } else {
            ...
            _meta_svc->get_server_state()->wait_all_task();
            LOG_AND_RETURN_NOT_OK(error_f, rpc.response().err, "set_app_envs failed.");
        }
    }

    ddebug("Sync policies to app envs succeeded.");
    return dsn::ERR_OK;
}
