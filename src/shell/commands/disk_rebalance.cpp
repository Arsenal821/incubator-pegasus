...
void get_node_addr(const dsn::rpc_address &ip, bool resolve_ip, std::string &node_addr)
{
    if (resolve_ip && dsn::utils::hostname(ip, &node_addr)) {
        return;
    }

    node_addr = ip.to_std_string();
}
...
{
    // disk_capacity [-n|--node replica_server(ip:port)][-o|--out file_name][-j|-json]
    // [-d|--detail][-r|--resolve_ip]
    const std::set<std::string> &params = {"n", "node", "o", "out"};
    const std::set<std::string> &flags = {"j", "json", "d", "detail", "r", "resolve_ip"};
    ...
    for (const auto &err_resp : err_resps) {
        ...
        get_node_addr(err_resp.first, resolve_ip, node_addr);

        if (query_detail_info) {
            dsn::utils::table_printer disk_printer(node_addr);
            ...
            node_printer.add_row(node_addr);
...
bool query_disk_replica(command_executor *e, shell_context *sc, arguments args)
{
    ...
    // file_name][-j|--json][-r|--resolve_ip]
    const std::set<std::string> &params = {"n", "node", "a", "app", "o", "out"};
    const std::set<std::string> &flags = {"j", "json", "r", "resolve_ip"};
    argh::parser cmd(args.argc, args.argv, argh::parser::PREFER_PARAM_FOR_UNREG_OPTION);
    if (!validate_cmd(cmd, params, flags)) {
        return false;
    }
    ...
    for (const auto &err_resp : err_resps) {
        ...
        for (const auto &disk_info : resp.disk_infos) {
            ...
            for (const auto &rs : disk_info.holding_primary_replicas) {
                primary_count += rs.second.size();
            }

            for (const auto &rs : disk_info.holding_secondary_replicas) {
                secondary_count += rs.second.size();
            }
            disk_printer.add_row(disk_info.tag);
