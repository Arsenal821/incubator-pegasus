...
void replica_bulk_loader::broadcast_group_bulk_load(const bulk_load_request &meta_req)
{
    ...
    for (const auto &hp : _replica->_primary_states.membership.hp_secondaries) {
        if (hp == _stub->_primary_host_port)
            continue;
                                                   ...
                                                   const group_bulk_load_response &resp)
{
    ...
    if (req.config.ballot != get_ballot()) {
        LOG_ERROR_PREFIX(
            "recevied wrong group_bulk_load response from {}({}), request ballot = {}, "
            "current ballot = {}",
            req.hp_target_address.to_string(),
            req.target_address.to_string(),
            req.config.ballot,
    ...
    _replica->_primary_states.secondary_bulk_load_states[req.hp_target_address] =
                                         ...
                                         const std::string &local_dir)
{
        ...
        remote_dir, local_dir, bulk_load_constant::BULK_LOAD_METADATA, fs, file_size);
    {
        ...
        if (err != ERR_OK) {
            try_decrease_bulk_load_download_count();
            _download_status.store(err);
            LOG_ERROR_PREFIX("parse bulk load metadata failed, error = {}", err.to_string());
            return;
        }
    }
...
void replica_bulk_loader::report_group_download_progress(/*out*/ bulk_load_response &response)
{
    ...
    for (const auto &target_hp : _replica->_primary_states.membership.hp_secondaries) {
        ...
        LOG_INFO_PREFIX("secondary = {}, download progress = {}%, status={}",
                        ...
                        s_progress,
                        s_status);
        response.group_bulk_load_state[_stub->get_dns_resolver()->resolve_address(target_hp)] =
            secondary_state;
...
void replica_bulk_loader::report_group_is_paused(bulk_load_response &response)
{
    ...
    for (const auto &target_hp : _replica->_primary_states.membership.hp_secondaries) {
        ...
        LOG_INFO_PREFIX(
            "secondary = {}, bulk_load is_paused = {}", target_hp.to_string(), is_paused);
        response.group_bulk_load_state[_stub->get_dns_resolver()->resolve_address(target_hp)] =
            secondary_state;
        response.hp_group_bulk_load_state[target_hp] = secondary_state;
